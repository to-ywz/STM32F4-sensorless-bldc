...
当前 `HAL_ADCEx_InjectedConvCpltCallback()` 已读取 ADC1 注入 Rank 1~3，并按实时 VDDA、三相独立零点、采样电阻和放大倍数换算为三相电流后送入 VOFA+。启动阶段会采集 2048 组样本完成三相零点校准；当前仍未接入过流保护和电流环。
...
当前正式控制分支启动时会在驱动器关闭状态下采集 2048 组 ADC1 三相电流样本，分别生成 U/V/W 零点偏置；标定结果仅保存在运行时 RAM，尚未接入过流保护和电流环。
...
- 继续评估 ADC1 三相电流零点校准的热稳定性，记录不同温度和上电状态下的 offset 漂移。
