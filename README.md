...
本轮三相电流采样测试、ADC 触发点调整和验证结果见 [`Docs/FOC_L3_4_三相电流采样验证记录.md`](Docs/FOC_L3_4_三相电流采样验证记录.md)。

软件按扇区选择两相并重构第三相的实现记录见 [`Docs/FOC_L4_3_软件电流选择与第三相重构实现记录.md`](Docs/FOC_L4_3_软件电流选择与第三相重构实现记录.md)。
...
当前采样架构分为三档：ADC1 注入组由 TIM1_CH4 以 16 kHz 同步采集三相电流；ADC3 在 `MONITOR` 模式以 1 kHz 规则组 DMA 采集三相端电压和 VBUS，仅用于 VOFA、趋势观察和模拟前端验证；ADC1 规则组以软件触发约 10 Hz 采集 VREFINT。启动时会先丢弃首个 VREFINT 结果并平均 16 次有效采样，再保持 SD 关闭 400 ms 后完成 2048 点三相原始 ADC 零点校准。

STM32F407 的 ADC 规则组触发选择不包含 TIM6_TRGO。因此此版本的 MONITOR 模式由 TIM6 的 1 kHz 更新中断启动一次 ADC3 四 Rank DMA 序列；TIM6 仍是采样节拍源，但不是 ADC 的硬件 EXTSEL 源。ADC3 的启动入口、采样节拍和 DMA 完成回调相互分离，以便以后替换为 TIM1 派生的硬件同步源（`SYNC` 模式）。

直接 BEMF 在低速时幅值很小，不能作为零速或极低速位置估计的主要来源。六步 BEMF 过零检测应在 PWM 安静窗口同步采样。常规 FOC 观测器主要依赖同步电流、施加电压和母线电压，不一定要求三相模拟 BEMF 以电流环频率采集；如果未来将实测相电压用于观测器，采样相位必须与电流环严格对应，但可采用 8 kHz 等确定性分频，不必强制为 16 kHz。
