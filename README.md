...
本阶段问题分析、零点校准、电流钳验证和三电阻电流重构总结见 [`Docs/FOC_L3_6_本阶段问题分析与解决总结.md`](Docs/FOC_L3_6_本阶段问题分析与解决总结.md)。
...
| 任务 | 优先级 | 当前安排 |
|---|---|---|
| 软件过流保护 | ⭐ 高 | 初版已接入，待上板验证阈值和触发行为 |
| 电流增益标定 | 中 | 使用现有电流钳和已知电流条件完成 |
| 硬件过流保护 | 低 | 后续功率板或硬件版本升级时考虑，作为独立硬件关断链路 |

软件过流保护初版已经接入，但在完成上板测试前，三相电流保护仍不能视为最终定型。硬件过流保护最终应优先接入 TIM1 BKIN 或驱动器专用故障关断输入。

软件过流保护初始参数：限值 `5.0 A`，连续 `2` 个 PWM 周期超限后触发。触发后关闭 PWM、拉低驱动器 SD 并锁存故障，复位后重新校准才能恢复。
